...
public:
    ...
    void deallocate(T* ptr) {
        std::size_t index = (reinterpret_cast<char*>(ptr) - buffer) / sizeof(T);
        used[index] = false;
    }
};
...
template <typename DataType, std::size_t N, typename Compare = decltype(cmp_int), typename Func = decltype(print_int)>
class BinTree {
    BNode<DataType> sentinel;
    BNode<DataType>* root = &sentinel;
    Compare comp;
    Func func;
    StaticHeap<BNode<DataType>, N> heap;
    ...
    void rb_insert_fixup(BNode<DataType>* z) {
        while (z->parent->getColor() == BNode<DataType>::RED) {
